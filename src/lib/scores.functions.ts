  ...
  .handler(async ({ data, context }) => {
    ...
    const result = streamText({
      ...
      system:
        ...
        "【谱号与音高，最容易出错，务必逐条执行】：" +
        "1. 读音之前，先看清每一行每个谱表开头的谱号。高音谱号(G谱号)五条线从下到上是 E4 G4 B4 D5 F5；" +
        "低音谱号(F谱号)五条线从下到上是 G2 B2 D3 F3 A3，四个间从下到上是 A2 C3 E3 G3，上加一线是中央C(C4)。" +
        "同一个位置在两种谱号里是不同的音，绝不能用高音谱号的读法去读低音谱表。" +
        "2. ABC 的音名是绝对音高，clef=bass 不会改变音名：中央C写 C，C3 写 C,，G2 写 G,,，C5 写 c。" +
        "左手写在低音谱表里的音，大多应落在 C,, 到 C 之间。" +
        "3. 曲中如果换了谱号（例如左手临时换成高音谱号），在换谱号处写 [K:clef=treble] 或 [K:clef=bass]，并按新谱号读音。" +
        "4. 写完后自查：如果左手大部分音高于中央C，或需要很多上加线，说明你把低音谱号按高音谱号读了，必须重读左手。" +
